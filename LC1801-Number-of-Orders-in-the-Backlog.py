...
executed, and that buy order will be removed from the backlog. Else,
...
class Solution:
    ...
    def getNumberOfBacklogOrders(self, orders: List[List[int]]) -> int:
        ...
        for price, amount, orderType in orders:
            while amount and orderType == 0 and sell and sell[0][0] <= price:
                deals = min(amount, sell[0][1])
                amount -= deals
                sell[0][1] -= deals
                if not sell[0][1]:
                    sell.pop(0)
            while amount and orderType == 1 and buy and buy[-1][0] >= price:
                deals = min(amount, buy[-1][1])
                amount -= deals
                buy[-1][1] -= deals
                if not buy[-1][1]:
                    buy.pop()
            if amount and orderType == 0:
                bisect.insort(buy, [price, amount])
            if amount and orderType == 1:
                bisect.insort(sell, [price, amount])
        return (sum(b[1] for b in buy) + sum(s[1] for s in sell)) % self.modulo
...
if __name__ == '__main__':
    ...
    print(f'Running tests for getNumberOfBacklogOrders')
    run_tests(Solution().getNumberOfBacklogOrders, correct_answers)