...
*/
// My very first piece of code in Java, please forgive me any errors/style issues.
class Solution {
    public int countBalls(int lowLimit, int highLimit) {
        ...
        for (int n=lowLimit; n <= highLimit; n++){
            ...
            if (digitSum.containsKey(sum)){
                digitSum.put(sum, digitSum.get(sum) + 1);
            } else {
                digitSum.put(sum, 1);
            };
        }
        // find the largest value in digitSum
        int maxValue = 0;
        for (int count: digitSum.values()){
            if (count > maxValue){
                maxValue = count;
            };
        }
